...
import { RANDOM_GAME_NFT_CONTRACT_ABI, RANDOM_GAME_NFT_CONTRACT_ADDRESS } from "../constants";
import { subgraphQuery } from "../utils";
import { FETCH_ALL_GAME, FETCH_CREATED_GAME } from "../queries";
...
export default function Home() {
   ...
   const getProviderOrSigner = async (needSigner = false) => {
      ...
      if (needSigner) {
         const signer = web3Provider.getUncheckedSigner();
         return signer;
      }
      return web3Provider;
   };
   const getOwner = async () => {
      try {
         ...
         const code = await provider.getCode(RANDOM_GAME_NFT_CONTRACT_ADDRESS);
   ...
   const checkIfGameStarted = async () => {
      try {
         ...
         if (_gameStarted) {
            ...
            const _gameArray = await subgraphQuery(FETCH_ALL_GAME());

            _gameArray.games.forEach((game) => {
               _logs.push(
                  `Previous game has ended with ID: ${game.id}`,
                  `Winner is: ${game.winner} 🎉 `
               );
            });
            _logs.push(`Waiting for host to start new game....`);
   ...
   return (
      <div>
         ...
         <div className={styles.main}>
            <div>
               <h1 className={styles.title}>Welcome to Random Winner Game! @{connectedAddress}</h1>
               <div className={styles.description}>
                  Its a lottery game where a winner is chosen at random and wins the entire lottery
                  pool
               </div>
               {renderButton()}
               {logs &&
